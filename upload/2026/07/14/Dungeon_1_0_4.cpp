...
vector<Skill> ALL_SKILLS = {
    ...
    {"经验加成", "获得经验增加", PASSIVE, 0, 3, 0, 0, 0.20f}, // 每级+20%
    {"金币加成", "获得金币增加", PASSIVE, 0, 3, 0, 0, 0.20f},
    {"魔力再生", "每回合额外回复MP", PASSIVE, 0, 3, 0, 0, 1.0f},             // 每级+1 mp/回合
    {"冲刺", "向移动方向快速移动3/4/5格", ACTIVE, 0, 3, 3, 5, 3},            // 冷却3, 蓝耗5, 距离3
    {"烈焰风暴", "对周围5x5敌人造成伤害", ACTIVE, 0, 3, 5, 8, 1.0f},         // 基础伤害为攻击力 150%
    {"治疗术", "回复最大生命百分比", ACTIVE, 0, 3, 7, 10, 0.15f},            // 基础回复18%
    {"护盾", "获得可吸收伤害的护盾，持续5回合", ACTIVE, 0, 3, 10, 10, 0.2f}, // 吸收量=最大生命25%
    {"狂暴", "临时增加攻击力，持续3回合", ACTIVE, 0, 3, 5, 7, 0.3f},         // 增加攻击力35%
    {"铁壁", "临时增加防御力，持续3回合", ACTIVE, 0, 3, 5, 7, 0.3f},         // 增加防御力35%
    {"冻结", "冻结视野内所有敌人", ACTIVE, 0, 3, 10, 10, 2},                  // 冻结3/4/5回合
};
...
private:
    ...
    void applySkill(int slot)
    {
        ...
        else if (sk.name == "烈焰风暴")
        {
            ...
            for (auto &e : enemies)
            {
                if (e.hp > 0 && abs(e.pos.x - player.pos.x) + abs(e.pos.y - player.pos.y) <= 2)
                {
        ...
        else if (sk.name == "铁壁")
        {
            ...
            buffDefenseTurns = 3; // 独立计时
            message = "Defense boosted!";
        }
        else if (sk.name == "冻结")
        {
            frozenCounter = (int)(sk.value + lvMult); // 基础 2 + 等级，最高5
    ...
    void rangedAttack()
    {
        ...
        int cost = max(1, (int)(player.maxHp * 0.02 + 1));
        if (player.hp <= cost)
        {
    ...
    void movePlayer(int dx, int dy)
    {
        ...
        if (itemIt != items.end())
        {
            ...
            {
            case POTION:
                player.hp = min(player.maxHp, int(player.hp * 1.1 + 15));
                break;
            case ATK_BOOST:
                player.atk += 2;
                break;
