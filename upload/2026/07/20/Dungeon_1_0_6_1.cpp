...
vector<Skill> ALL_SKILLS = {
    ...
    // 主动技能
    {"冲刺", "向移动方向快速移动4/5/6格", ACTIVE, 0, 3, 3, 15, 3},
    {"烈焰风暴", "对周围敌人造成伤害", ACTIVE, 0, 3, 5, 40, 1.0f},            // value=1.0, 增量2.0 → 3/5/7倍
    {"治疗术", "回复最大生命百分比", ACTIVE, 0, 3, 15, 100, 0.10f},           // value=0.10, 增量0.10 → 0.20/0.30/0.40
    {"护盾", "获得可吸收伤害的护盾，持续6回合", ACTIVE, 0, 3, 11, 70, 0.25f}, // value=0.25, 增量0.15 → 0.40/0.55/0.70
    {"狂暴", "临时增加攻击力，持续6回合", ACTIVE, 0, 3, 8, 50, 0.20f},        // value=0.20, 增量0.10 → 0.30/0.40/0.50
    {"铁壁", "临时增加防御力，持续6回合", ACTIVE, 0, 3, 8, 50, 0.20f},        // 同上
    {"冻结", "冻结所有敌人", ACTIVE, 0, 3, 11, 80, 3},                        // value=3, 增量1 → 4/5/6回合
    {"闪现", "向移动方向瞬间移动，可穿墙", ACTIVE, 0, 3, 8, 40, 6},           // value=6, 增量3 → 9/12/15格
    {"导弹打击", "随机重击附近敌人，有伤害下限", ACTIVE, 0, 3, 6, 60, 6.0f},  // value=6, 增量3
    {"熔岩吐息", "向方向扇形喷火，降低敌人防御", ACTIVE, 0, 3, 6, 50, 2.0f},  // value=2, 增量1
};
...
private:
    ...
    Point libraryPos;
    int libraryCost;  // 当前购买价格
    int libraryCount; // 本层已购买次数（用于计算价格）
    ...
    void placeBoss()
    {
        ...
        boss.symbol = 'B';
        int baseHp = (int)(12 + rand() % 3 + floorNumber * 11);
        int baseAtk = (int)(6 + rand() % 2 + floorNumber * 6);
        int baseDef = (int)(4 + rand() % 2 + floorNumber * 3);
        boss.maxHp = (int)(baseHp * (3.5f + floorNumber * 0.3f));
        boss.atk = (int)(baseAtk * (2.2f + floorNumber * 0.8f));
    ...
    {
        items.clear();
        int itemCount = 5 + floorNumber; // 保持原公式，不设硬上限
        for (int i = 0; i < itemCount; ++i)
        {
            ...
            for (int attempt = 0; attempt < 100; ++attempt)
            {
                ...
                if (map[y][x] != FLOOR || (player.pos.x == x && player.pos.y == y) ||
                    (shopAvailable && shopPos.x == x && shopPos.y == y) ||
                    (libraryAvailable && libraryPos.x == x && libraryPos.y == y) ||
                    (stairs.x == x && stairs.y == y))
                    continue;

                // 查找该位置是否已有物品
                auto it = find_if(items.begin(), items.end(), [&](const MapItem &item)
                                  { return item.pos == Point{x, y}; });
                if (it != items.end())
                {
                    // 同类型则堆叠
                    if (it->type == mi.type)
    ...
    void initFloor()
    {
        ...
        // 图书馆生成（30%概率，不与商店/玩家/楼梯重叠，且不在出生房间）
        libraryAvailable = (rand() % 100) < 30;
        if (libraryAvailable)
        {
            do
            {
                libraryPos.x = rand() % MAP_WIDTH;
                libraryPos.y = rand() % MAP_HEIGHT;
            } while (map[libraryPos.y][libraryPos.x] != FLOOR || libraryPos == player.pos ||
                     (shopAvailable && libraryPos == shopPos) || libraryPos == stairs ||
                     inRoom(spawnRoomCenter, spawnRoomW, spawnRoomH, libraryPos));
            libraryCost = 30;
            libraryCount = 0;
        }
        else
    ...
    void applySkill(int slot)
    {
        ...
        if (sk.name == "冲刺")
        {
            ...
            int dist = (int)(sk.value + lvMult); // 4/5/6
        ...
        {
            int dmg = (int)(player.atk * (sk.value + lvMult * 2.0f)); // 1.0 + level*2.0 → 3/5/7
            for (auto &e : enemies)
            {
        ...
        {
            float ratio = sk.value + lvMult * 0.10f; // 0.10 + level*0.10 → 0.20/0.30/0.40
        ...
        {
            float ratio = sk.value + lvMult * 0.15f; // 0.25 + level*0.15 → 0.40/0.55/0.70
        ...
        {
            float ratio = sk.value + lvMult * 0.10f; // 0.20 + level*0.10 → 0.30/0.40/0.50
        ...
        {
            float ratio = sk.value + lvMult * 0.10f;
        ...
        {
            frozenCounter = (int)(sk.value + lvMult); // 3 + level → 4/5/6
        ...
        else if (sk.name == "闪现")
        {
            ...
            int dist = (int)(sk.value + lvMult * 3.0f); // 6 + level*3 → 9/12/15
            int nx = player.pos.x, ny = player.pos.y;
            for (int step = 0; step < dist; ++step)
            {
                int tx = nx + lastDx, ty = ny + lastDy;
        ...
        else if (sk.name == "导弹打击")
        {
            // 收集附近敌人（距离 <= 3）
            vector<Entity *> Nearby, Far;
            for (auto &e : enemies)
            {
                if (e.hp > 0)
                {
            ...
            if (!Nearby.empty())
            {
                target = Nearby[rand() % Nearby.size()];
            }
            else if (!Far.empty())
            {
                target = Far[rand() % Far.size()];
            }
            if (!target)
            {
                message = "No target!";
            ...
            float dmgMult = sk.value + lvMult * 3.0f; // 6/9/12
            float hpFloor = 0.2f + lvMult * 0.1f;     // 0.3/0.4/0.5
            int rawDmg = (int)(player.atk * dmgMult);
            int minDmg = (int)(target->maxHp * hpFloor);
            int finalDmg = max(calcEnemyDamage(rawDmg, *target), minDmg); // 经减伤后取至少下限
            target->hp -= finalDmg;
            message = "Missile strike for " + to_string(finalDmg) + " damage!";
            if (target->hp <= 0)
                handleEnemyDeath(target);
        }
        else if (sk.name == "熔岩吐息")
        {
            if (lastDx == 0 && lastDy == 0)
            {
            ...
            float dmgMult = sk.value + lvMult * 1.0f; // 2.0 / 3.0 / 4.0
            float defReduce = lvMult * 0.10f;         // 0.10 / 0.20 / 0.30
            int dx = lastDx, dy = lastDy;
            int lx = -dy, ly = dx; // 左方向
            int rx = dy, ry = -dx; // 右方向
            for (int depth = 1; depth <= 3; ++depth)
            {
                // 中心点坐标
                int cx = player.pos.x + dx * depth;
                int cy = player.pos.y + dy * depth;
                // 中心点越界或撞墙则整个吐息停止
                if (cx < 0 || cx >= MAP_WIDTH || cy < 0 || cy >= MAP_HEIGHT || map[cy][cx] == WALL)
                    break;

                // 处理该层的所有格子：偏移 -depth .. depth
                for (int off = -depth; off <= depth; ++off)
                {
                    ...
                    auto it = find_if(enemies.begin(), enemies.end(), [&](const Entity &e)
                                      { return e.hp > 0 && e.pos == Point{nx, ny}; });
                    if (it != enemies.end())
                    {
                        // 先降低防御（永久）
                        it->def = max(1, (int)(it->def * (1.0f - defReduce)));
                        it->originalDef = max(1, (int)(it->originalDef * (1.0f - defReduce)));
                        // 再计算伤害（使用降低后的防御）
                        int dmg = calcEnemyDamage((int)(player.atk * dmgMult), *it);
                        it->hp -= dmg;
                        if (it->hp <= 0)
                            handleEnemyDeath(&*it);
                    }
    ...
    void movePlayer(int dx, int dy)
    {
        ...
        if (itemIt != items.end())
        {
            int amount = itemIt->amount; // 堆叠数量
            string itemName;
            switch (itemIt->type)
            {
            case POTION:
            {
                // 每瓶药水恢复 15 HP，但可连续恢复多瓶（或一次性恢复总量）
                int totalHeal = 0;
                for (int i = 0; i < amount; ++i)
                {
                    int before = player.hp;
                    player.hp = min(player.maxHp, (int)(player.hp + player.maxHp * 0.1 + 15));
                    totalHeal += player.hp - before;
                    if (player.hp == player.maxHp)
                        break; // 血满则停止使用
                }
                message = "Picked up " + to_string(amount) + " Potion(s), restored " + to_string(totalHeal) + " HP.";
                break;
            }
            case ATK_BOOST:
            {
                player.atk += 2 * amount;
                message = "Picked up " + to_string(amount) + " ATK Boost(s)! ATK increased by " + to_string(2 * amount) + ".";
                break;
            }
            case DEF_BOOST:
            {
                player.def += 1 * amount;
                message = "Picked up " + to_string(amount) + " DEF Boost(s)! DEF increased by " + to_string(amount) + ".";
                break;
            }
            case GOLD:
            {
                player.gold += itemIt->amount;
                message = "Picked up " + to_string(amount) + " Gold!";
                break;
            }
            }
            items.erase(itemIt);
        ...
        if (shopAvailable && player.pos == shopPos)
            shop();
        if (libraryAvailable && player.pos == libraryPos)
            library();
    }
    ...
    void library()
    {
        ...
        while (inLibrary)
        {
            system("cls");
            cout << "========== LIBRARY ==========\n";
            cout << "Ancient knowledge awaits...\n";
            cout << "Current price: " << libraryCost << " gold (per level)\n";
            cout << "Your gold: " << player.gold << "\n";
            cout << "Your level: " << player.level << "\n\n";
            cout << "1. Purchase 1 level\n";
            cout << "2. Leave library\n";
            if (!message.empty())
                cout << ">> " << message << " <<\n";
            cout << "Choose: ";
            char c;
            ...
            if (c == '1')
            {
                if (player.gold >= libraryCost)
                {
                    player.gold -= libraryCost;
                    // 给予 1 级经验
                    int reqExp = calcreqExp();
                    player.exp += reqExp;
                    checkLevelUp(); // 触发升级
                    // 价格递增
                    libraryCost = (int)(libraryCost * 1.5);
                    libraryCount++;
                    message = "You feel wiser! Gained 1 level.";
                }
                else
                {
                    message = "Not enough gold! Need " + to_string(libraryCost) + " gold.";
                }
            }
            else
            {
    ...
    void skillSelection()
    {
        ...
        if (candidates.empty())
        {
            int goldBonus = floorNumber * 100;
            player.gold += goldBonus;
            // 短暂显示提示（需要一个短暂暂停，可以使用 message 或直接输出）
            system("cls");
            cout << "No skills available.\n";
            cout << "You receive " << goldBonus << " gold instead!\n";
    ...
    void draw()
    {
        ...
        if (libraryAvailable && libraryPos.x != -1)
            display[libraryPos.y][libraryPos.x] = 'L';
            ...
            if (player.pos == stairs)
            {
                ...
                if (floorNumber % 10 == 0)
                {
                    ...
                    if (ch == 'y' || ch == 'Y')
                    {
                        ...
                        player.mp = min(player.maxMp, player.mp + player.maxMp / 2 + 5);
                        skillSelection(); // 选择技能
                        saveGame();
                        initFloor();
                    }
